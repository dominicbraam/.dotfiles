" -----------------------------------------------------------------------------
" Name:         SAGA
" Description:  A dark theme with colourful shades of pastel 
" -----------------------------------------------------------------------------
...
let g:colors_name = 'SAGA'
" }}}
" Configuration: {{{
let s:configuration = {}
let s:configuration.style = get(g:, 'SAGA', 'dark')
let s:configuration.transparent_background = get(g:, 'SAGA_transparent_background', 0)
let s:configuration.menu_selection_background = get(g:, 'SAGA_menu_selection_background', 'green')
let s:configuration.disable_italic_comment = get(g:, 'SAGA_disable_italic_comment', 0)
let s:configuration.enable_italic = get(g:, 'SAGA_enable_italic', 0)
let s:configuration.cursor = get(g:, 'SAGA_cursor', 'auto')
let s:configuration.current_word = get(g:, 'SAGA_current_word', get(g:, 'SAGA_transparent_background', 0) == 0 ? 'grey background' : 'bold')
" }}}
" Palette: {{{
"
